...
def summary(parsed: dict) -> str:
    """Generate a compact human-readable summary for LLM context injection (~200 tokens)."""
    lines = [f"Scan: {parsed.get('scan_info', {}).get('scanner_args', 'nmap')}"]
    for host in parsed.get("hosts", []):
        lines.append(f"\nHost: {host['ip']} ({host['hostname']}) — {host['state']}")
        if host.get("os_guess"):
            lines.append(f"  OS: {host['os_guess']} ({host['os_accuracy']}% confidence)")
        open_svcs = [s for s in host.get("ports", []) if s["state"] == "open"]
        for svc in open_svcs:
            version_str = svc.get("full_version", "").strip() or f"{svc.get('service', 'unknown')}"
            lines.append(
                f"  {svc['port']}/{svc['protocol']}  {svc.get('service', 'unknown'):<12}  {version_str}"
            )
